...
In the article \citep{wu2017capturing} they use a Kalman filter for smooth estimations of a billiards game.

In \citep{8273755} they present an adaptive kalman filter, which estimates Q and R values. 
...
Depending on the hardware we also might go with a proprietary GPGPU framework such as CUDA. In case we should decide to use this \citep{nickolls2008scalable} is an introduction into this topic.
...
state of the art error analysis and the uncertainties of measurement with \citep{taylor1997introduction}. 

\citep{allen1971mean} provides a good explanation why in predictions it is better to use the MSE instead of the LSE.
